                  ...
                  => 사용자 요청
                     ...
                     2) 화면 변경 => <a> ==> GET 방식
                        데이터를 받는 파일명?key=값&key=값...
                                              = 구분자
               => GET : URL주소에 노출 : id,pwd... (session에 저장 후 처리하는게 좋다)
                  => 클라이언트 / 서버
                  ...
                  2. 추가, 수정 => 데이터베이스
                  3. 예약 => 예약과 관련된 테이블을 컬럼
                            =====================
                            
                  동작방식
                  1. 사용자 요청
                     여기의 경우 : detail.jsp?no=<%=vo.getNo()%>&page=<%=curpage%>"><%=vo.getSubject() 이부분임
                     링크 => 화면변경
                  2. 요청된 데이터를 받기
                     no / page
                  3. 해당 데이터 베이스를 연결 == DAO 작업이 있다는말!
                  4. 처리된 결과를 받아서 화면 출력
