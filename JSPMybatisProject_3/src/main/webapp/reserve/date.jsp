  ...
  <table class="table">
    ...
    <tr class="danger">
      ...
      <c:forEach var="i" begin="1" end="${lastday }">  
       <c:if test="${i==1 }"><%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<!DOCTYPE html>
<html>
    ...
    <tr class="danger">
      ...
      <c:forEach var="i" begin="1" end="${lastday }">  
       <c:if test="${i==1 }">
        ...
        <c:if test="${rday[i]==0}">
         <td class="text-center" data-year="${year }" data-month="${month }">${i }</td>
        </c:if>
        <c:set var="week" value="${week+1 }"/>
        <c:if test="${week>6 }">
          <c:set var="week" value="0"/>
