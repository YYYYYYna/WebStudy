        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (count === 0) {
                    ...
                    // 아이디가 사용 가능한 경우에만 확인 버튼 보이기
                    $('#okBtn').show();
                } else {
                    $('#result').html('<font color="red">' + id + '는(은) 이미 사용중인 ID입니다</font>')
                    $('#okBtn').hide(); // 아이디가 이미 사용 중인 경우 확인 버튼 감추기
                    $('#id').val("");
                    $('#id').focus();
                }
            }
  ...
  <div class="container">
   <div class="row">
    <table class="table">
     ...
     <tr>
       <td class="text-center"><input type="button" class="btn btn-sm btn-danger" value="확인" id="okBtn"></td>
     </tr>
    </table>
   </div>
  </div>
