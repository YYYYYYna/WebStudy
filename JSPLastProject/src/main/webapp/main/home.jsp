...
<body>
	...
	<div class="wrapper row3">
		<main class="container clear">
			...
			<div class="flexslider carousel basiccarousel btmspace-80">
				<ul class="slides">
					<c:forEach var="vo" items="${aList }">
						<li>
							<figure>
								<img class="radius-10 btmspace-10" src="${vo.poster}">
								<figcaption>
									<a href="../food/food_before_detail.do?fno=+${fno }">${vo.name}</a>
								</figcaption>
							</figure>
						</li>
					</c:forEach>
			...
			<h2 class="sectiontitle">최신 방문 업체</h2>
			<div class="inline">
				<c:forEach var="kvo" items="${klist }" varStatus="s">
					<c:if test="${s.index<9 }">
						<img class="radious-10" src="${kvo.poster }"
							style="width: 100px; height: 100px;" title="${kvo.name }">
					</c:if>
				</c:forEach>
			</div>
			<!-- / main body -->
